...
<form:form id="data_form" modelAttribute="module" action="${pageContext.request.contextPath}/sys-module/update" cssClass="form-horizontal" role="form" method="post">
<div class="modal-body">
		...
		<div class="form-group form-group-sm">
			<label class="col-md-3 control-label">菜单编码</label>
			<div class="col-md-6">
				<input id="code" name="code" value="${module.code }" class="form-control"  ${action == 'update' ? 'readonly="true"' : '' }/>
...
<script>
	...
	$(function(){ 
		$('#data_form').validate({
			...
			messages : {
				name : "请输入菜单名称",
				code: {remote:'此菜单编码已存在，请重新输入',required:"请输入菜单编号"},
			},
			rules : {
				code : { required : true,
					<c:if test="${action == 'add' }">
					remote : {
						url:"${pageContext.request.contextPath}/sys-module/check_module_code.json",
						data : {
							code: function(){
								return $('#code').val();
							}
