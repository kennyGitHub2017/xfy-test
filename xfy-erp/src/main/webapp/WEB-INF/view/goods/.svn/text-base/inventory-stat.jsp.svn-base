...
<c:set scope="request" var="sysPage" value="inventory_stat" />

<c:set var="pageTitle" value="库存状态查询" />
							...
							<div class="table-toolbar">
								<form id="search_form" class="col-md-12 form-horizontal" role="form" search="1">
									...
									<div class="row">
										...
										<div class="col-md-4" id="category_div_id">
											<div class="form-group form-group-sm">
									...
									<div class="col-md-4" id="count_div_id">
										<div class="form-group form-group-sm">
											<label class="control-label col-md-4">库存数量</label>
											<div class="col-md-8">
...
$(function(){
	...
	var $datatable = $("#list_table").dataTable({
		...
		ajax: {
			type: 'post',
			url: "${pageContext.request.contextPath }/inventory/stat-json",
			data: function (d) {
		...
		drawCallback: function(settings) {
			...
			for (var i = 0; i < data.length; i++) {
				var row = data[i];
				total += (row.hisAvgPrice ? row.hisAvgPrice : 0) * (row.allCount ? row.allCount : 0);
			}
	...
	$("#export_form_btn").click(function(){
		var path = '${pageContext.request.contextPath}/inventory/exportStat';
