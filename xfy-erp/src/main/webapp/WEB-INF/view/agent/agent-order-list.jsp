...
<c:set scope="request" var="sysPage" value="agent-order" />
...
<c:if test="${not empty invalidrequest}">
...
<c:if test="${empty invalidrequest}">
	<tiles:putAttribute name="page-content">
			<div class="row">
				<div class="col-md-12">
							...
							<div class="portlet-body">
								...
								<form:form id="searchform" search="1" modelAttribute="search" method="post" class="form-horizontal" role="form">
									...
									<div class="row">
										...
										<div class="col-md-3">
											<div class="form-group form-group-sm">
												<label  class="col-md-3 control-label">卖家</label>
												<div class="col-md-9">
			...
			$("#searchbtn").click(function(){
				...
				if (val!=null){
					...
					$.ajax({
						...
						type:'post',
						data: {sellerId:null,userId:userId}, 
						...
						success:function(data){
							...
							if (jsondata.result){
								...
								$("label[for='skuTypeSearch6']").html(jsondata.s8);
								$("label[for='skuTypeSearch7']").html(jsondata.s9);
								$("label[for='skuTypeSearch8']").html(jsondata.s1);
								$("label[for='skuTypeSearch9']").html(jsondata.s2);
							}
						}
					});
				}
					...
					columns: [
						{
						 ...
						 {data:function(data){
					    	 ...
					    	 var retStr = data.orderPlatform+"<br>"+data.id;
		  ...
		  //二级分类select
		    ...
		    function formatitem(order) {
		    	...
		    	for(var i=0;i<items.length;i++){
		    		...
		    		var ftd = '<td orderid="{0}" sku="{1}" itemId="{2}"><br>&nbsp;&nbsp;&nbsp;&nbsp;<input type="checkbox" name="itemId" value="{2}-{3}-{4}"/></td>'.format(order.id,(items[i].sku==null?'':items[i].sku) ,itemId,order.id,order.orderPlatform);
		    		var sku = '<td><input type="text" style="width:100px" value="{0}" /></td>'.format( (items[i].sku==null?items[i].itemSku:items[i].sku));
